...
- (void)selectUserOk
{
    if (_selectedUser) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提醒" message:@"请先选择会签人员" delegate:self cancelButtonTitle:@"好的" otherButtonTitles:nil, nil];
        [alert show];
    }
}
...
//    _selectedUser = userCell.detailTextLabel.text;
    _selectedUser = [_userArray[indexPath.row] objectForKey:@"userName"];
//    _searchBar.placeholder = [NSString stringWithFormat:@"您选择了:%@ 会签",[_userArray[indexPath.row] objectForKey:@"name"]];
    self.title = [NSString stringWithFormat:@"您选择了:%@ 会签",[_userArray[indexPath.row] objectForKey:@"name"]];
}
...
//    _searchBar.placeholder = [NSString stringWithFormat:@"您选择了:%@ 会签",_selectedUser];
    self.title = [NSString stringWithFormat:@"您选择了:%@ 会签",name];
}
